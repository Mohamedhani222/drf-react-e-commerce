import {
  ...
  PRODUCT_ADD_COPON_FAIL,
  PRODUCT_ADD_COPON_REQUEST,
  PRODUCT_ADD_COPON_SUCCESS,
  PRODUCT_TOP_FAIL,
  PRODUCT_TOP_REQUEST,
  PRODUCT_TOP_SUCCESS,
...
export const listProductsAdmin = (keyword='') => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`/api/products/admin${keyword}`, config);
...
export const listProductsAdminDetails = (id) => async (dispatch, getState) => {
  try {
...
export const AddCopon = (orderID ,copon) => async (dispatch, getState) => {
  try {
    dispatch({
      type: PRODUCT_ADD_COPON_REQUEST,
    });
    ...
    const { data } = await axios.post(
      `/api/products/addcopon/${orderID}/`,
      copon,
      config
    );

    dispatch({
      type: PRODUCT_ADD_COPON_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: PRODUCT_ADD_COPON_FAIL,
      payload:
        error.response && error.response.data.detail
          ? error.response.data.detail
          : error.message,
